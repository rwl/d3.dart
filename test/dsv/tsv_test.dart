...
import 'package:d3/js/dsv.dart' as dsv;

tsvTest() {
  group('on a sample file', () {
    test('invokes the callback with the parsed tsv', () {
      dsv.tsv('../data/sample.tsv', callback: (err, tsv) {
        expect(
            tsv,
            equals([
              {'Hello': '42', 'World': '"fish"'}
  ...
  group('on a file that does not exist', () {
    test('invokes the callback with null when an error occurs', () {
      dsv.tsv('//does/not/exist.tsv', callback: (err, tsv) {
        expect(tsv, isNull);
      });
    });
  });
  ...
  group('parse', () {
    ...
    test('parses mac newlines', () {
      expect(
          parse('a\tb\tc\r1\t2\t3\r4\t5\t"6"\r7\t8\t9'),
          equals([
            {'a': '1', 'b': '2', 'c': '3'},
            {'a': '4', 'b': '5', 'c': '6'},
            {'a': '7', 'b': '8', 'c': '9'}
