...
main() {
  ...
  group('selectAll(div)', () {
    group('on a simple page', () {
      var div = new Selection.selector('body')
        .selectAll('div')
          .data([0, 1])
            .enter()
              .append('div');
      ...
      test('selects the first matching element', () {
        ...
        expect(span.nodeAttr('class'), equals('first'));
      });
      test('propagates parent node to the selected elements', () {
        var span = div.select('span');
        expect(parentNode(span[0]), isNotNull);
        expect(parentNode(span[0]), same(div.node.parentNode));
      ...
      test('can select via function', () {
        var dd = [], ii = [], tt = [];
        var s = div.data(['a', 'b']).selectFunc((node, d, i, j) {
          dd.add(d); ii.add(i); tt.add(node);
